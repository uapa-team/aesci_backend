        ...
        for i in range(len(courseCode)):	
            if pandas.isnull(courseCode[i]) != True:
                ...
                for y in range(len(indicatorsCode)):
                    if isTheNextindicatorOfAnotherCourse == False:
                ...
                if groupCoLists[i]:
                    for x in range(len(groupCoLists[i])):
                        if not groupCoLists[i]:
                            print("there is not")
                        else:
                            for z in range(len(currentCourseIndicators)):
                                ...
                                currentIndicatorGroup = IndicatorGroup.objects.filter(numGroup=groupCoLists[i][x], performanceIndicator=currentPerformanceIndicator.idPerformanceIndicator)
                                if not currentIndicatorGroup:
                                    currentMaxIndicatorGroup += 1
                                    obj, _ = IndicatorGroup.objects.get_or_create(idIndicatorGroup=currentMaxIndicatorGroup, performanceIndicator=currentPerformanceIndicator, numGroup=groupCoLists[i][x])
