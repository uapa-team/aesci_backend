...
#        try:
        if EvaluationAssignment.objects.filter(indicatorAssignment=instance.idIndicatorAssignment, assignmentStudent=self.context['request'].query_params["assignmentStudent"]).exists():
            response["grade"] = EvaluationAssignment.objects.filter(indicatorAssignment=instance.idIndicatorAssignment, assignmentStudent=self.context['request'].query_params["assignmentStudent"])[0].codeMeasure
        else:
            response["grade"] = -1
#        except:
